...
- 이 저장소는 공개(public)입니다. 커밋 메시지·문서·코드·주석에 **특정 회사명이나 이 저장소의 사용 맥락을 직접 드러내는 키워드**를 넣지 않습니다.
...
- **확장(향후, 설계 초안 있음)**: 캐시, 정규화 실패 격리, 중복 상품 병합, 다중 통화, 예약 대행. 설계 초안은 [docs/extensions.md](docs/extensions.md)에 있으며, 여력이 생기면 하나씩 구현합니다.
