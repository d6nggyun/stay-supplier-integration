...
public record SearchCriteria(LocalDate checkIn, LocalDate checkOut, int adults, int children) {
    ...
    public SearchCriteria {
        ...
        if (!checkIn.isBefore(checkOut)) {
            throw new IllegalArgumentException("checkIn은 checkOut보다 이전이어야 합니다.");
        }
        if (adults <= 0) {
            throw new IllegalArgumentException("adults는 0보다 커야 합니다.");
        }
        if (children < 0) {
            throw new IllegalArgumentException("children은 0 이상이어야 합니다.");
        }
    }
