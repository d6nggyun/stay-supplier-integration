...
재시도와 서킷 브레이커는 필수(타임아웃·병렬·부분 실패·실패 판정)를 안정화한 뒤 확장으로 도입했습니다. 재시도는 일시적 실패(타임아웃·연결 실패·5xx)만 대상으로 하고, 서킷은 공급사별로 둡니다. 상세 설계·설정은 [resilience.md](resilience.md)를 참고하세요.
