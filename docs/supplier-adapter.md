...
어댑터는 외부 응답 변화에 관대하게 대응합니다. 응답 DTO는 필요한 필드만 선언하고, 공급사가 응답에 필드를 추가해도 역직렬화가 깨지지 않도록 모르는 필드는 무시합니다. 외부 변화를 흡수하는 경계로서 우리가 쓰는 값만 뽑아내는 것이 목적입니다.
...
방침은 **해당 항목을 결과에서 제외하되 버리지 않고 격리(dead-letter)로 남기는 것**입니다. 오케스트레이터의 코드→내부 식별자 치환 지점에서 매핑을 찾지 못하면, 공급사·코드·사유(`MAPPING_NOT_FOUND`)·시각을 `normalization_failure` 테이블에 기록합니다(추후 매핑·어댑터 개선 분석용). 기록은 **best-effort**로, 실패해도 검색 결과(부분 성공)에는 영향이 없습니다. DB 쓰기는 리액티브 스레드를 막지 않도록 검색 종료 후에 일괄 저장합니다.

> 캐시된 결과에 치환 실패 항목이 있으면 캐시 히트마다 격리 레코드가 반복될 수 있습니다(치환 실패는 드문 방어적 경로). 사유별 dedup은 향후 개선으로 둡니다.
